"""Audit the 24-cell ICASSP scope, including SEED-V and PhysioNet-MI extensions."""
...
DEFAULT_TABLE = ROOT / "paper_table_manifest_v4.csv"
EXPECTED_SEEDS = {"3407", "2024", "2027"}
EXPECTED_DATASETS = {"FACED", "ISRUC", "SEED-V", "PhysioNet-MI"}
...
def audit(table_path: Path) -> dict:
    ...
    if len(rows) != 24:
        errors.append(f"v4 paper table must contain exactly 24 rows, found {len(rows)}")
    ...
    for row in rows:
        ...
        if dataset not in EXPECTED_DATASETS or condition not in EXPECTED_CONDITIONS:
            errors.append(f"{identity}: dataset/condition is outside v4 scope")
        if row.get("seed") not in EXPECTED_SEEDS:
            errors.append(f"{identity}: seed is not in {sorted(EXPECTED_SEEDS)}")
        expected_new_run = "yes" if dataset in {"SEED-V", "PhysioNet-MI"} else "no"
        if row.get("required_new_run") != expected_new_run:
            errors.append(
                f"{identity}: required_new_run must be {expected_new_run}"
            )
        if condition == "specialist_augmented_full":
            ...
            expected_protocol = f"paper_protocol_{dataset.lower()}_v1".replace("seed-v", "seedv").replace("physionet-mi", "physionet_mi")
    ...
    return {
        ...
        "scope": "prespecified_extension_v4",
    }
...
def main() -> None:
    ...
    if args.strict and not result["passed"]:
        raise SystemExit("v4 paper scope audit failed")

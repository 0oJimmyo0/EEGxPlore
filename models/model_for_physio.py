...
class Model(nn.Module):
    def __init__(self, param):
        ...
        print(f"[PhysioNet-MI] attnres_variant = {param.attnres_variant}")
        print(f"[PhysioNet-MI] attnres_gated = {param.attnres_gated}")
        print(f"[PhysioNet-MI] attnres_gate_init = {param.attnres_gate_init}")
        print(f"[PhysioNet-MI] attnres_start_layer = {param.attnres_start_layer}")
        if getattr(param, 'moe', False):
            print(
                f"[PhysioNet-MI] MoE (typed_capacity_domain): top-{param.moe_num_layers} layers, "
        ...
        if param.use_pretrained_weights:
            ...
            if param.attnres_variant == 'none' and not getattr(param, 'moe', False):
                print('[PhysioNet-MI] Baseline mode: strict foundation load')
            elif getattr(param, 'moe', False):
                print('[PhysioNet-MI] MoE mode: partial load + dense FFN warm-start into experts')
            else:
                print(f"[PhysioNet-MI] AttnRes mode ({param.attnres_variant}): partial foundation load")
            print(f"[PhysioNet-MI] Backbone tensors marked pretrained: {len(self.pretrained_param_names)}")

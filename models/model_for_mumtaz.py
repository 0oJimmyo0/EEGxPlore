...
class Model(nn.Module):
    def __init__(self, param):
        ...
        print(f"[Mumtaz2016] attnres_variant = {param.attnres_variant}")
        print(f"[Mumtaz2016] attnres_gated = {param.attnres_gated}")
        print(f"[Mumtaz2016] attnres_gate_init = {param.attnres_gate_init}")
        print(f"[Mumtaz2016] attnres_start_layer = {param.attnres_start_layer}")
        if getattr(param, 'moe', False):
            print(
                f"[Mumtaz2016] MoE (typed_capacity_domain): top-{param.moe_num_layers} layers, "
        ...
        if param.use_pretrained_weights:
            ...
            if param.attnres_variant == 'none' and not getattr(param, 'moe', False):
                print('[Mumtaz2016] Baseline mode: strict foundation load')
            elif getattr(param, 'moe', False):
                print('[Mumtaz2016] MoE mode: partial load + dense FFN warm-start into experts')
            else:
                print(f"[Mumtaz2016] AttnRes mode ({param.attnres_variant}): partial foundation load")
            print(f"[Mumtaz2016] Backbone tensors marked pretrained: {len(self.pretrained_param_names)}")
